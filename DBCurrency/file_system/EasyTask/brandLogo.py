...
class BrandLogo:
    ...
    def get_download_urls(self):
        ...
        sql_sentence = "select br_uuid, br_logourl from product$brand_import where br_logourl is not null and br_logourl_c is null"
    ...
    def update_local_url(self, server_file_url, br_uuid):
        oracle_conn = cx_Oracle.connect(B2B_Oracle_Url)
        cursor = oracle_conn.cursor()
        sql_sentence = "update product$brand_import set br_logourl_c='{}' where br_uuid='{}'".format(server_file_url, br_uuid)
        cursor.execute(sql_sentence)
